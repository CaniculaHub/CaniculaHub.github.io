...
struct led_dev{
	...
	struct class *cls;
	struct device *dev;
	struct resource *res; //»ñÈ¡µ½µÄÄÚ´æ×ÊÔ´
	void *reg_base; //±íÊ¾ÎïÀíµØÖ·Ó³ÉäÖ®ºóµÄÐéÄâµØÖ·
};
...
int led_pdrv_probe(struct platform_device *pdev)
{
	...
	/*
		a£¬×¢²áÉè±¸ºÅ£¬²¢ÇÒ×¢²áfops--ÎªÓÃ»§Ìá¹©Ò»¸öÉè±¸±êÊ¾£¬Í¬Ê±Ìá¹©ÎÄ¼þ²Ù×÷io½Ó¿Ú
		b£¬ ´´½¨Éè±¸½Úµã
		c£¬ ³õÊ¼»¯Ó²¼þ
					ioremap(µØÖ·);  //µØÖ·´ÓpdevÐèÒª»ñÈ¡
					readl/writle();
		d£¬ÊµÏÖ¸÷ÖÖio½Ó¿Ú£º xxx_open, xxx_read, ..
