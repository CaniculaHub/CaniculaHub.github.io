...
![](http://wx4.sinaimg.cn/mw690/b00a7483gy1fqp9w30i2nj209h08zq2v.jpg)
...
![](http://wx2.sinaimg.cn/mw690/b00a7483gy1fqp9w5lko0j20k307ndg9.jpg)
...
![](http://wx1.sinaimg.cn/mw690/b00a7483gy1fqp9w7ijtcj207t06xwei.jpg)
...
- 若`sem_op < 0`，请求 sem_op 的绝对值的资源。
  ...
  - 当相应的资源数不能满足请求时，这个操作与`sem_flg`有关。
    ...
    - sem_flg 没有指定`IPC_NOWAIT`，则将该信号量的semncnt值加1，然后进程挂起直到下述情况发生：
...
- 若`sem_op == 0`，进程阻塞直到信号量的相应值为0：
  ...
  - 如果信号量的值不为0，则依据`sem_flg`决定函数动作：
    ...
    - sem_flg没有指定`IPC_NOWAIT`，则将该信号量的semncnt值加1，然后进程挂起直到下述情况发生：
      ...
      2. 此信号量被删除，函数smeop出错返回EIDRM；
      3. 进程捕捉到信号，并从信号处理函数返回，在此情况将此信号量的semncnt值减1，函数semop出错返回EINTR在`semctl`函数中的命令有多种，这里就说两个常用的：
